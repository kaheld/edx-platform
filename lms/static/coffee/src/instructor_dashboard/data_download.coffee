  ...
  constructor: (@$section) ->
    ...
    @$oe_data_download_btn.click (e) =>
      ...
      $.ajax
        ...
        error: std_ajax_err =>
          @clear_oe_display()
          @$oe_request_response_error.text "Error getting open response assessment data.  Please notify technical support if this persists."
        success: (data) =>
          @clear_oe_display()
          if data.success == true
            @$oe_display_text.html "<a href='" + data.file_url + "' target='_blank'>Click to download data</a>"
          else
            @$oe_request_response_error.text "No open response assessment data is available.  Please check again later."
